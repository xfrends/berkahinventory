<form id="parsley-form" class="form-horizontal" novalidate name="form_edit">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <label class="control-label"><?php echo lang('Village') ?> <?php echo lang('ID') ?></label>
        <?php
            $new_id = substr($record->id, strlen($record->district_id), strlen($record->id));
        ?>
...
<script>
    ...
    $("#parsley-form").parsley().on('field:validated',function(){}).on('form:submit', function(){
        var link = "<?php echo base_url('area/update_village/'.$record->id) ?>",
